...
export default function AdminLayout({ children }: AdminLayoutProps) {
    const { auth, unreadOrdersCount = 0, accountingCashBalance, flash } = usePage<
   ...
   >().props;
    ...
    const accountingSubNav: { href: string; label: string }[] = [
        ...
        { href: '/admin/accounting/receipt-payment-report', label: 'Receipt & Payment' },
        { href: '/admin/accounting/income-expenditure-report', label: 'Income & Expenditure' },
        { href: '/admin/accounting/balance-sheet-report', label: 'Balance Sheet' },
        { href: '/admin/accounting/product-analysis-report', label: 'Product Analysis Report' },
    ];
    ...
    return (
        <div className="flex min-h-screen bg-muted/30">
            ...
            {showFlash && (
                <div
                    className={`fixed left-0 right-0 top-0 z-50 flex items-center justify-between gap-4 px-4 py-3 text-sm shadow-md sm:left-64 sm:right-0 ${
                        flash?.error ? 'bg-red-100 text-red-800 dark:bg-red-950/80 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-950/80 dark:text-green-200'
                    }`}
                >
                ...
                <div className="flex h-full flex-col">
                    ...
                    <nav className="flex-1 space-y-1 overflow-y-auto p-4">
                        {mainNavItems.map((item) => {
                            ...
                            const isActive = item.href === '/admin/accounting' ? (currentUrl === '/admin/accounting' || currentUrl === '/admin/accounting/') : checkIsActive(item.href, item.exact);
                                ...
                                <div key={item.href}>
                                    <Link
                                        ...
                                        className={`relative flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                                            item.href === '/admin/accounting' && isAccountingSection
                                    ...
                                    {item.href === '/admin/accounting' && isAccountingSection && (
                                        <div className="ml-4 mt-1 space-y-0.5 border-l-2 border-border pl-3">
                                            {accountingSubNav.map((sub) => {
                                                ...
                                                const isSubActive = sub.href === '/admin/accounting' ? path === '/admin/accounting' : path === subPath || path.startsWith(subPath + '/');
                                                return (
                                                    <Link
                                                        key={sub.href}
                                                        href={sub.href}
                        ...
                        <div className="border-t border-border px-4 py-3">
                            <div className="rounded-lg bg-muted/50 px-3 py-2 text-sm">
                                ...
                                <div className="font-semibold text-foreground">
                                    ৳{accountingCashBalance.toLocaleString('en-BD', { minimumFractionDigits: 2 })}
                                </div>
                            </div>
                        </div>
                    )}
            ...
            {/* Main Content */}
            <main className={`flex-1 min-h-screen ${showFlash ? 'pt-12' : ''}`}>
