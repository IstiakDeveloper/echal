...
export default function BalanceSheetReport({
    ...
    const maxRows = Math.max(
        Math.max(report.assets.length, 1),
        Math.max(report.liabilities_equity.length, 1),
    );

    const assetRows =
        report.assets.length > 0
            ? report.assets
            : [{ name: 'No assets', code: '', balance: 0 }];
    const liabilityRows =
        report.liabilities_equity.length > 0
            ? report.liabilities_equity
            : [{ name: 'None', code: '', balance: 0 }];
            ...
            <AdminLayout>
                <div className="report-container space-y-6">
                    ...
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                            form.get('/admin/accounting/balance-sheet-report', {
                    ...
                    <div className="report-paper overflow-hidden rounded border border-border bg-card">
                        <table className="report-table w-full">
                            <thead>
                                <tr>
                                    <th className="report-th report-divider w-1/2 text-left">
                                        Assets (Left)
                                    </th>
                                    <th className="report-th w-1/2 text-left">
