...
export default function ProductAnalysisReport({
    report,
    filters,
}: ProductAnalysisReportProps) {
    const form = useForm({
    ...
    return (
        <>
            ...
            <AdminLayout>
                <div className="report-container report-product-analysis-page space-y-6">
                    ...
                    <div className="no-print grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
                    ...
                    <p className="no-print text-muted-foreground">
                        {report.period_label}
                    </p>

                    <div className="report-paper report-product-analysis overflow-hidden rounded border border-border bg-card">
                        <div className="overflow-x-auto">
                            <table className="report-table product-analysis-table w-full">
                                <thead>
                                    <tr>
                                        <th
                                            colSpan={2}
                                            className="report-th bg-muted/80 text-center"
                                        >
                                            PRODUCT
                                        </th>
                                        <th
                                    ...
                                    <tr>
                                        <th className="report-th report-num">
                                            SL
                                        </th>
                                ...
                                <tbody>
                                    {report.rows.map((row) => (
                                        ...
                                        >
                                            ...
                                            <td className="report-td report-num text-right">
                                                {row.before_price > 0
                                                    ? `৳${fmt(row.before_price)}`
                                                    : '—'}
                                            </td>
                                            <td className="report-td report-num text-right">
                                                {row.before_value > 0
                                                    ? `৳${fmt(row.before_value)}`
                                                    : '—'}
                                            </td>
                                            <td className="report-td report-num text-right">
                                                {num(row.buy_qty)}
                                            ...
                                            <td className="report-td report-num text-right">
                                                {row.sale_subtotal > 0
                                                    ? `৳${fmt(row.sale_subtotal)}`
                                                    : '—'}
                                            </td>
                                            <td className="report-td report-num text-right">
                                                {row.sale_discount > 0
                                                    ? `৳${fmt(row.sale_discount)}`
                                                    : '—'}
                                            </td>
                                            <td className="report-td report-num text-right">
                                                {row.sale_total > 0
